...
function generateNMonths(n) {
    const data = []
    for (let i = 0; i < n; i++) {
        const date = new Date();
        date.setMonth(date.getMonth() - i);  // ลดเดือนทีละ 1
        data.push(date.toLocaleString('en-US', {
            month: 'short',  // แสดงชื่อเดือนสั้น (เช่น Jan, Feb)
            year: 'numeric'  // แสดงปี
        }));
    }
    return data.reverse();  // เรียงเดือนจากเก่ามาใหม่
}
